...
Senão se (idade < 18)
    Escrever ('Seu voto é facultativo!')
Senão
    Escrever ('Seu voto é obrigatório')
...
Classe Círculo Herda de FormaGeometrica:
    Atributos:
        - raio
...
INICIO
    ...
    Ler distanciaAlvo
    Ler tempoMaximo
    Ler velocidadeMaxima

    velocidadeInicial = 0
    tempo = 0

    Para i de 0 até tempoMaximo faça:
    ...
    Se distanciaPercorrida == distanciaAlvo
        Escrever ()"A distância alvo foi alcançada em" + tempo/60 "segundos")
    Senão Se tempo == tempoMaximo
            Escrever "O tempo máximo foi atingido"
...
Função MultiplicaçãoDeMatrizes(matrizA, matrizB):
    Se tamanho(matrizA[0]) ≠ tamanho(matrizB) # considerando que todas as linhas têm o mesmo número de colunas
    ...
    Senão:
        ...
        matrizResultado = novaMatriz(linhasA)
        para i de 0 até linhasA-1 faça:
            matrizResultado[i] = novaMatriz(colunasB)
...
Escrever("MultiplicaçãoDasMatrizes das matrizes:")
ImprimirMatriz(matrizSoma)
```
~~~
